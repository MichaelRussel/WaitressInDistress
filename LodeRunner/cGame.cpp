...
bool cGame::Init()
{
	...
	if (!cAssetManager::getInstance().Load())
		printf("Some images may missing");
	if (!cSound::getInstance().Load())
		printf("Some sounds may missing"); // debug kalau ada lagu hilang
...
std::string cGame::getHighScore() 
{
	std::string score;
	std::ofstream file;
	std::ifstream fileR;

	// create and write file if it doesn't exist
	if (!checkFile("Textures\\highscore.txt")) {
		file.open("Textures\\highscore.txt");
		score = "0";
		file << score;
		file.close();
	}
	// read file if it exist
		...
		fileR.open("Textures\\highscore.txt");
		fileR >> score;
		fileR.close();
	}
	return score;
...
std::string cGame::getMap()
{
	std::string map;
	std::ifstream fileR;

	//read file
	fileR.open("Textures\\map.txt");

	while (std::getline(fileR, map, '\0')) {
		std::getline(fileR, map);
	}
	fileR.close();
...
void cGame::setHighScore(std::string newHighScore) {
		std::ofstream file;
		file.open("Textures\\highscore.txt");
		file << newHighScore;
		file.close();
